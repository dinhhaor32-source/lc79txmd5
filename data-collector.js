...
// ============================================================================
// NEW: Historical Data Methods for Prediction Engine
// ============================================================================

/**
 * Get historical sessions for pattern matching
 * @param {number} limit - Maximum number of sessions to return
 * @returns {Array<Object>} - Historical sessions
 */
function getHistoricalSessions(limit = 200) {
  ...
  return lines
    .slice(-limit) // Last N sessions
    .map(l => JSON.parse(l))
    .reverse(); // Most recent first
}

/**
 * Save prediction result to predictions.jsonl
 * @param {Object} prediction - Prediction record
 */
function savePrediction(prediction) {
  const PRED_FILE = path.join(__dirname, "predictions.jsonl");
  try {
    fs.appendFileSync(PRED_FILE, JSON.stringify(prediction) + "\n");
  } catch (err) {
    console.error("[DATA] Error saving prediction:", err.message);
  }
}
